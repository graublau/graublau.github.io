    ...
    if (item) {
      ...
      getEpicsPromise.then(() => {
        getChannelsPromise.then(() => {
          ...
          function replaceNullWithHyphen(value) {
            return (value !== null && value.trim() !== '') ? value : '-';
          }
          
          listElementDrop.innerHTML = '<li id="content' + item.id + '" class="modal"><div class="back"><a href="javascript:void(0);" id="go_back_from_drop"><i class="gg-chevron-left"></i></a></div><div class="close"><a href="#"><i class="gg-close"></i></a></div><div class="contentcontainer">' + replaceNullWithHyphen(locale_pub_date_time_start) + '&nbsp;-&nbsp;' + replaceNullWithHyphen(locale_pub_date_time_end) + '&nbsp;(' + replaceNullWithHyphen(item.timezone) + ')</div><h1>' + replaceNullWithHyphen(item.title) + '</h1>' + '<div class="copy">' + replaceNullWithHyphen(item.text) + '</div><div class="descr"><img id="mainImage" src="' + imageUrl + '" /></div><div class="descr">Channel</div>' + replaceNullWithHyphen(channeltitle) + '</div><div class="descr">Images, Videos, Assets</div>' + replaceNullWithHyphen(item.assets) + '<div class="descr">Topic:</div><a href="#topic' + replaceNullWithHyphen(item.epic) + '">' + replaceNullWithHyphen(epictitle) + '</a><div class="descr">Channel Owner</div>' + replaceNullWithHyphen(channelowner) + '<div class="descr">Topic Owner</div>' + replaceNullWithHyphen(epicowner) + '<div class="descr">Assigned Editor</div>' + replaceNullWithHyphen(item.editor) + '<div class="descr">Last edited by</div>' + replaceNullWithHyphen(item.last_edited_by) + '<div style="margin-top:2em;"><input type="checkbox" name="finalized" id="finalized" class="cta"> Final and Published / Scheduled</div><div class="editcontainer"><a href="#editcontent' + item.id + '" class="button">Edit</a>&nbsp;<a href="#duplicatecontent' + item.id + '" class="button">Duplicate</a>&nbsp;<input type="button" name="delete" id="deleteDrop" value="Delete" onclick="DeleteDrop()" class="cta"/></div></div></li>';        
...
var calendarEl = document.getElementById('calendar');
  var calendar = new FullCalendar.Calendar(calendarEl, {
    ...
    initialView: window.innerWidth < 768 ? 'resourceTimelineToday' : 'resourceTimelineMonth',
    firstDay: 1, // Monday
    views: {
